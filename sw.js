...
const CACHE_NAME = 'eduorbit-v3.5'; // Version bump = old cache deleted instantly
    ...
    caches.keys().then(cacheNames => {
      return Promise.all(
        cacheNames.map(cache => {
          ...
          console.log('[SW v3.5] Deleting old cache:', cache);
      ...
      return self.clients.matchAll({ type: 'window' }).then(clients => {
        clients.forEach(client => {
          client.postMessage({ type: 'SW_UPDATED', version: '3.5' });
        });
      });
    })
  );
